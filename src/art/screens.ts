...
function castle(ctx: Ctx, g: Game): void {
  ...
  if (c.hint) drawBubble(ctx, c.hint.text, 640, 200, 'none')
  else if (c.floor === 0 && c.t < 4 && c.state === 'walk') drawBubble(ctx, ['Climb the ladders to the throne room!', 'Tap a ladder to climb it. Gray ladders are tricks!'], 640, 330, 'none')
  drawFrame(ctx)
}
